...
load depth_10.mat
figure;
imagesc(depth_array);%o que � que isto faz?
title('Depth image');
%XY sao as coordenadas da imagem, index � a depth(cordenada da perfundidade)
...
imagesc(B);%R,G,B
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Perguntar ao professor que cores ficam em 
%colormap(flipud(colormap))
title(['\color{Red}R Image                                                    ' ...
    '\color{Green}G Image                                                     ' ...
    '\color{Blue}B Image ']);
...
%aB
figure;
%colormap(gray); mostra a imagem em cinzento
%%%%%%%%%%%% Converte para cinzento
%IM2 = rgb2gray(IM);
%imshow(IM2);
imagesc(depth_array);%o que � que isto faz?
